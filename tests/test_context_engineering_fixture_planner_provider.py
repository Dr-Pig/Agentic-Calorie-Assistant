...
def test_fixture_planner_provider_emits_structured_plan_without_keyword_route() -> None:
    provider = FixtureContextEngineeringPlannerProvider(model_profile="fixture-fast")
    case = _case("ce-stress-001")

    artifact = provider.plan_case(case)
    serialized = json.dumps(artifact)

    assert artifact["artifact_type"] == "advanced_product_lab_fixture_planner_trace"
...
def test_fixture_planner_trace_covers_selected_ce_stress_cases() -> None:
    trace = build_context_engineering_fixture_planner_trace(
        case_ids=["ce-stress-001", "ce-stress-006", "ce-stress-007", "ce-stress-012"]
    )

    assert trace["artifact_type"] == "advanced_product_lab_fixture_planner_suite_trace"
...
def test_fixture_planner_output_blocks_forbidden_fields_and_mutation_flags() -> None:
    blockers = fixture_planner_output_blockers(
        {
    ...
    assert blockers == [
        "planner_output.forbidden_field:keyword_route",
        "planner_output.forbidden_field:raw_user_text",
