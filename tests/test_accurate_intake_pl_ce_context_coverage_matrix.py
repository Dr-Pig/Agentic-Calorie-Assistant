...
def test_context_coverage_matrix_blocks_inconsistent_fake_provider_handoff_count() -> None:
    from app.composition.accurate_intake_pl_ce_context_coverage_matrix import (
        build_pl_ce_context_coverage_matrix_artifact,
    )
    ...
    assert artifact["status"] == "blocked"
    assert (
        "fake_provider_context_smoke.manager_handoff_scenario_count_mismatch"
        in artifact["blockers"]
    )
