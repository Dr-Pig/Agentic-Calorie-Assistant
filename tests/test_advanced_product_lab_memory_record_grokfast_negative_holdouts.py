...
def test_negative_holdout_cases_cover_block_and_downrank_without_oracle_payload() -> None:
    ...
    assert by_id["negative_eggplant_block"]["expected_candidate"]["strength"] == "block"
    ...
    assert payload["output_contract"]["discarded_profile_artifacts_are_not_cases"] is True
...
def test_negative_holdout_grader_blocks_wrong_eggplant_strength() -> None:
    from app.advanced_shadow_lab.product_lab_memory_record_grokfast_extraction import (
        run_memory_record_grokfast_extraction_diagnostic,
    )
    from app.advanced_shadow_lab.product_lab_memory_record_grokfast_extraction_cases import (
    ...
    artifact = run_memory_record_grokfast_extraction_diagnostic(
        cases=cases,
        provider=_FakeExtractionProvider(cases, corrupt_eggplant=True),
        provider_mode="fake_provider_contract_test",
        live_invoked=False,
        provider_profile_id=ADVANCED_LAB_DIAGNOSTIC_PROFILE_ID,
        case_suite="negative_holdout",
    ...
    assert (
        "case:negative_eggplant_block."
        "strength_mismatch"
    ) in artifact["blockers"]
    assert artifact["semantic_hardening_allowed"] is False


def test_negative_holdout_cli_fake_mode_writes_five_case_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["negative_holdout_summary"]["ignored_case_count"] == 0
...
class _FakeExtractionProvider:
    def __init__(self, cases: list[dict[str, Any]], *, corrupt_eggplant: bool = False) -> None:
        self.cases = cases
        self.corrupt_eggplant = corrupt_eggplant
    ...
    async def complete_with_trace(self, **kwargs: object) -> tuple[dict[str, object], dict[str, object]]:
        ...
        for case in self.cases:
            ...
            if self.corrupt_eggplant and case["case_id"] == "negative_eggplant_block":
                strength = "downrank"
            else:
