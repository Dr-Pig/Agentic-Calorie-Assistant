...
def test_proposal_shaping_live_diagnostic_script_fake_mode(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    output = tmp_path / "rescue_proposal_shaping_provider_diagnostic.json"

    result = subprocess.run(
        [
            sys.executable,
            str(root / "scripts" / "run_rescue_proposal_shaping_live_diagnostic.py"),
            "--provider-mode",
            "fake",
            "--output",
            str(output),
