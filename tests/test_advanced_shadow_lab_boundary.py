...
def test_advanced_shadow_lab_manifest_is_dormant_and_non_semantic() -> None:
    ...
    assert manifest["slice_mode"] == [
        "contract",
        "fixture_only",
        "diagnostic_only",
        "offline_runtime",
    ]
    assert manifest["capability_domains_planned"] == ADVANCED_LAB_CAPABILITY_DOMAINS
    ...
    assert manifest["production_manager_mount_allowed"] is False
    ...
    assert manifest["proactive_sent"] is False
    assert manifest["notification_delivery_allowed"] is False
    assert manifest["fooddb_expansion_allowed"] is False
...
def test_advanced_shadow_lab_manifest_records_locked_execution_decisions() -> None:
    manifest = build_advanced_shadow_lab_manifest()

    assert manifest["live_provider_policy"] == {
    ...
    assert manifest["proactive_surface_policy"] == {
        "chat_only": True,
        "inbox_mirror_allowed": False,
        "push_line_or_os_notification_allowed": False,
        "scheduler_delivery_allowed": False,
    }
    assert manifest["fooddb_policy"] == {
        "expansion_allowed": False,
        "self_use_required_before_expansion": True,
        "fixtures_or_approved_packets_only": True,
    }
    assert manifest["merge_back_dormancy_contract"] == {
        "required_false_flags": REQUIRED_MAINLINE_FALSE_FLAGS,
        "mainline_activation_requires_separate_pr": True,
    }
    assert manifest["non_claims"]["not_live_provider_activation"] is True
    assert manifest["non_claims"]["not_kimi_activation"] is True
    assert manifest["non_claims"]["not_fooddb_expansion"] is True


def test_manifest_required_false_flags_are_false_on_boundary_manifest() -> None:
    manifest = build_advanced_shadow_lab_manifest()

    for flag in REQUIRED_MAINLINE_FALSE_FLAGS:
