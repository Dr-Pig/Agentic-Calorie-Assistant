...
def test_product_pages_renderer_source_map_covers_chat_today_body_sources() -> None:
    ...
    assert "#body-budget-dashboard" in body["selectors"]
    assert "#deficit-summary-card" in body["selectors"]
    assert "#deficit-active-target" in body["selectors"]
    assert "#deficit-consumed" in body["selectors"]
    assert "#deficit-remaining" in body["selectors"]
    assert "#deficit-latest-weight" in body["selectors"]
    assert "#weekly-progress-card" in body["selectors"]
    assert "#weekly-progress-list" in body["selectors"]
    assert "#weekly-deficit" in body["selectors"]
    assert "#weekly-weight-delta" in body["selectors"]
    assert "#weekly-logged-days" in body["selectors"]
    assert "#effective-budget-card" in body["selectors"]
    assert "#effective-budget" in body["selectors"]
    assert "#effective-adjustment" in body["selectors"]
    assert "#effective-base" in body["selectors"]
    ...
    assert "payload.active_daily_target_kcal" in body["backend_fields"]
    assert "payload.consumed_kcal" in body["backend_fields"]
    assert "payload.remaining_kcal" in body["backend_fields"]
    assert "payload.latest_weight_kg" in body["backend_fields"]
    assert "payload.days" in body["backend_fields"]
    assert "payload.estimated_weekly_deficit_kcal" in body["backend_fields"]
    assert "payload.weight_delta_kg" in body["backend_fields"]
    assert "payload.logged_day_count" in body["backend_fields"]
    assert "payload.runtime_effective_budget_kcal" in body["backend_fields"]
    assert "payload.runtime_adjustment_total_kcal" in body["backend_fields"]
    assert "payload.base_budget_kcal" in body["backend_fields"]
