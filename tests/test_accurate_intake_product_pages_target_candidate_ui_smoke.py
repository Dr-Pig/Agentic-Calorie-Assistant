...
def _passing_report() -> dict[str, object]:
    return {
        ...
        "browser": {
            "fetch_sequence": [
                {
                    "url": "/accurate-intake/chat-history?user_id=target-candidate-ui",
                    "method": "GET",
                }
            ],
            "storage": {"localStorageKeys": [], "sessionStorageKeys": []},
            "product_page_text": "Possible references 2 possible matches luwei milk tea",
        },
    }
...
def test_target_candidate_ui_smoke_missing_playwright_is_blocked_not_readiness(
    ...
    assert report["smoke_id"] == "accurate_intake_product_pages_target_candidate_ui_smoke_v1"
...
def test_target_candidate_ui_smoke_validator_accepts_read_only_candidate_surface() -> None:
...
def test_target_candidate_ui_smoke_validator_rejects_missing_candidate_render() -> None:
    report = _passing_report()
    report["target_candidate_surface_checked"] = False
    report["target_candidate_count_rendered"] = 0
...
def test_target_candidate_ui_smoke_validator_rejects_selection_or_semantic_ownership() -> None:
    report = _passing_report()
...
def test_target_candidate_ui_smoke_validator_rejects_readiness_or_evidence_truth_claims() -> None:
    report = _passing_report()
...
def test_target_candidate_ui_smoke_validator_rejects_debug_text_storage_or_missing_fetch() -> None:
    report = _passing_report()
...
def test_target_candidate_ui_smoke_validator_rejects_unexpected_semantic_fetch_or_provider_call() -> None:
    report = _passing_report()
    ...
    report["browser"]["fetch_sequence"] = [
        ...
        },
        {
            "url": "/estimate",
            "method": "POST",
            "body": '{"text":"remove tofu","allow_search":false}',
        },
        {"url": "/accurate-intake/debug?user_id=target-candidate-ui", "method": "GET"},
    ]

    status, blockers = module._validate(report)

    assert status == "fail"
    assert "manager_provider_call_count_not_zero" in blockers
    assert "unexpected_fetch:POST /estimate" in blockers
    assert "unexpected_fetch:GET /accurate-intake/debug" in blockers


def test_target_candidate_ui_smoke_validator_requires_exact_provider_count_and_fetch_path() -> None:
    report = _passing_report()
    del report["manager_provider_call_count"]
    report["browser"]["fetch_sequence"] = [
        {"url": "/not-chat-history?next=/accurate-intake/chat-history", "method": "GET"}
    ]
    ...
    assert "manager_provider_call_count_not_zero" in blockers
    assert "required_fetch_missing:/accurate-intake/chat-history" in blockers
    assert "unexpected_fetch:GET /not-chat-history" in blockers


def test_target_candidate_ui_smoke_stays_out_of_fooddb_websearch_live_and_contract_boundaries() -> None:
