...
async def _provider_result_from_local_oracle(
    intent: Any,
    consumption: Any,
    *,
...
async def test_b2_pass2_oracle_parity_matches_local_oracle(case_factory) -> None:
    intent, consumption, clarify_support = case_factory()
    local_result, provider_result = await _provider_result_from_local_oracle(
        intent,
        consumption,
        clarify_support=clarify_support,
    )

    _assert_b2_oracle_parity(local_result, provider_result)


@pytest.mark.asyncio
async def test_b2_pass2_oracle_parity_fails_on_wrong_exactness_posture() -> None:
    ...
    _, provider_result = await _provider_result_from_local_oracle(
        intent,
        consumption,
        clarify_support=clarify_support,
...
@pytest.mark.asyncio
async def test_b2_pass2_oracle_parity_fails_on_wrong_likely_kcal() -> None:
...
@pytest.mark.asyncio
async def test_b2_pass2_oracle_parity_fails_on_wrong_followup_output() -> None:
...
@pytest.mark.asyncio
async def test_b2_pass2_oracle_parity_fails_when_forbidden_mutation_field_is_present() -> None:
    intent, consumption, clarify_support = _generic_anchor_case("\u6211\u5403\u4e86\u8336\u8449\u86cb")
    local_result = synthesize_b2_local_manager_pass2(intent, consumption, clarify_support=clarify_support)
    bad_payload = {
