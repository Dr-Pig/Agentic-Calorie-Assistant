...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 115
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 17,
        "generic_common_serving": 54,
        "listed_component": 44,
    }
    assert report["gap"]["packet_ready_item_count"] == 885
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 233,
        "generic_common_serving": 346,
        "listed_component": 306,
    }
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 885
