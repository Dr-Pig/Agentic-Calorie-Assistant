...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 140
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 22,
        "generic_common_serving": 64,
        "listed_component": 54,
    }
    assert report["gap"]["packet_ready_item_count"] == 860
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 228,
        "generic_common_serving": 336,
        "listed_component": 296,
    }
    assert report["gap"]["exact_macro_complete_count"] == 178
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 860
