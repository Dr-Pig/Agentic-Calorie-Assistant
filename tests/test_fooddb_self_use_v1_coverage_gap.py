...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 262
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 54,
        "generic_common_serving": 114,
        "listed_component": 94,
    }
    assert report["gap"]["packet_ready_item_count"] == 738
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 196,
        "generic_common_serving": 286,
        "listed_component": 256,
    }
    assert report["gap"]["exact_macro_complete_count"] == 146
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 738
