...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 434
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 86,
        "generic_common_serving": 194,
        "listed_component": 154,
    }
    assert report["gap"]["packet_ready_item_count"] == 566
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 164,
        "generic_common_serving": 206,
        "listed_component": 196,
    }
    assert report["gap"]["exact_macro_complete_count"] == 114
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 566
