...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 502
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 94,
        "generic_common_serving": 214,
        "listed_component": 194,
    }
    assert report["gap"]["packet_ready_item_count"] == 498
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 156,
        "generic_common_serving": 186,
        "listed_component": 156,
    }
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 498
