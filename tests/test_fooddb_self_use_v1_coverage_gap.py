...
def test_fooddb_self_use_v1_gap_reports_current_packet_ready_counts_against_1000_target() -> None:
    ...
    assert report["current"]["packet_ready_item_count"] == 366
    assert report["current"]["lane_counts"] == {
        "exact_item_card": 78,
        "generic_common_serving": 154,
        "listed_component": 134,
    }
    assert report["gap"]["packet_ready_item_count"] == 634
    assert report["gap"]["lane_counts"] == {
        "exact_item_card": 172,
        "generic_common_serving": 246,
        "listed_component": 216,
    }
...
def test_fooddb_self_use_v1_gap_cli_writes_roundtrippable_artifact(tmp_path: Path) -> None:
    ...
    assert artifact["gap"]["packet_ready_item_count"] == 634
