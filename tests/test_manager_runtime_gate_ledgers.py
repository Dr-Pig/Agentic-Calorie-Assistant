...
def test_manager_runtime_gate_ledger_records_small_slice_gate_order() -> None:
    ...
    assert gates["rt11b_final_response_quality_live_wall"]["status"] == "green"
    assert gates["rt11b_final_response_quality_live_wall"]["pass_type"] == "runtime_backed"
    assert gates["rt11b_final_response_quality_live_wall"]["depends_on"] == [
    ...
    assert gates["rt11c_renderer_input_basis_evidence_pack"]["status"] == "green"
    assert gates["rt11c_renderer_input_basis_evidence_pack"]["pass_type"] == "runtime_backed"
    assert gates["rt11c_renderer_input_basis_evidence_pack"]["depends_on"] == [
        "rt6_bootstrap_no_plan_body_closure",
        "rt8_overshoot_runtime_truth",
    ...
    assert gates["rt14d_single_case_tool_choice_live_probe"]["status"] == "green"
    assert gates["rt14d_single_case_tool_choice_live_probe"]["pass_type"] == "runtime_backed"
    assert gates["rt14d_single_case_tool_choice_live_probe"]["depends_on"] == [
        "rt14c_schema_contract_live_canary",
    ]
    assert gates["rt14e_context_conditioned_live_probe"]["status"] == "green"
    assert gates["rt14e_context_conditioned_live_probe"]["pass_type"] == "runtime_backed"
    ...
    assert gates["rt10c_exact_item_live_quality_probe"]["status"] == "green"
    assert gates["rt10c_exact_item_live_quality_probe"]["pass_type"] == "runtime_backed"
    assert gates["rt10c_exact_item_live_quality_probe"]["depends_on"] == [
        "rt10b_nutrition_estimate_quality_fake_provider",
        "rt14e_context_conditioned_live_probe",
    ]
    assert gates["rt10d_generic_optional_refinement_live_probe"]["status"] == "green"
    assert gates["rt10d_generic_optional_refinement_live_probe"]["pass_type"] == "runtime_backed"
    assert gates["rt10d_generic_optional_refinement_live_probe"]["depends_on"] == [
        "rt7d_optional_refinement_attach_boundary",
        "rt10b_nutrition_estimate_quality_fake_provider",
        "rt14e_context_conditioned_live_probe",
    ]
    assert gates["rt10e_blocking_clarify_correction_live_probe"]["status"] == "green"
    assert gates["rt10e_blocking_clarify_correction_live_probe"]["pass_type"] == "runtime_backed"
    assert gates["rt10e_blocking_clarify_correction_live_probe"]["depends_on"] == [
        "rt7a_correction_removal_runtime_boundary",
        "rt7b_blocking_clarify_pending_followup_boundary",
        "rt10b_nutrition_estimate_quality_fake_provider",
        "rt14e_context_conditioned_live_probe",
