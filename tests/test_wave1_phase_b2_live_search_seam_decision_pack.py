...
def test_decision_pack_includes_no_live_search_seam_and_does_not_auto_activate() -> None:
    ...
    assert pack["selected_option"] == "no_live_search_seam"
    assert pack["requires_human_decision"] is False
    assert pack["recommended_safe_default"] == "no_live_search_seam"
    assert pack["selection_reason"] == "trace_canary_has_blockers"
    assert pack["evidence_summary"]["failure_families"] == ["extract_mismatch"]


def test_clean_trace_canary_continues_trace_only_without_runtime_activation() -> None:
    ...
    assert pack["runtime_web_activation_approved"] is False
    assert pack["selected_option"] == "trace_only_canary_continues"
    assert pack["requires_human_decision"] is False
    assert pack["selection_reason"] == "trace_canary_clean_but_runtime_activation_not_approved"
...
def test_missing_tavily_token_artifact_keeps_web_deferred() -> None:
    ...
    assert pack["selected_option"] == "no_live_search_seam"
    assert pack["requires_human_decision"] is False
    assert pack["selection_reason"] == "tavily_live_not_invoked"
    assert pack["runtime_web_activation_approved"] is False
