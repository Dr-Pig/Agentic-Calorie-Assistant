...
- Stable repo branches are `main` and `codex/advanced-product-lab`. Do not create additional long-lived implementation branches for advanced-product work. Right-sized PR slices may be tracked and merged on the long-lived lab branch itself; branch multiplication is not architecture.
- Advanced product-lab work and Current Shell work must converge on the same Manager-style agent contract. Branch differences may change activation posture, evidence sources, and lab-only surfaces, but they must not create a second orchestration model, a second truth-owner map, or a second tool vocabulary.
- Current Shell manager/runtime contracts are the upstream alignment target. Advanced product-lab work must conform to Current Shell contract families and extend them additively for later-wave capabilities; do not make mainline contract owners realign to advanced-lab-local semantics.
- Treat `pass1/pass2`, scripted manager loops, or direct fixture runners as harness shapes only. The target runtime stance remains one bounded Manager-style ReAct loop with shared capability planning, shared tool registry, shared truth owners, and deterministic legality guards.
...
- Repeated or reusable meals are not memory-only truth. If a user repeatedly eats or cooks the same meal, model it as a shared product object such as `UserFoodEntity` or `RecurringMealTemplate`, with memory acting only as retrieval hint, recall summary, or promotion evidence. Do not collapse reusable-meal truth into free-text memory or provider-side cache semantics.
