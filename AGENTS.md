...
- Code-reference-first bootstrap is mandatory for implementation-sensitive mechanism work. Use repo truth first, then inspect relevant implementation code references before official-doc-only best-practice claims; official docs are normative API/framework evidence, not mechanism implementation proof. Record inspected reference paths, compared mechanisms, and `adopted_or_rejected_rationale` before planning or implementation. If no suitable code reference exists, state that explicitly, keep the slice narrow, and do not turn official-doc-only evidence into a mechanism-readiness claim.
