...
DEFAULT_OUTPUT = ROOT / "artifacts" / "accurate_intake_grokfast_websearch_candidate_packet_smoke.json"

WEBSEARCH_CANDIDATE_MANAGER_SYSTEM_PROMPT = (
    SINGLE_MANAGER_SYSTEM_PROMPT
    + "\nWebSearch candidate packet seam diagnostic: the user payload contains a read-only "
    "websearch_manager_evidence_packet_v1 from candidate-only SearchCandidatePacket records. "
    "Use only packet references. Do not create exact-card truth, do not produce nutrition item_results, "
    "do not write ledger state, and do not claim readiness.\n"
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run GrokFast Manager + WebSearch candidate packet seam smoke."
    )
    ...
    packet_artifact = _load_manager_packet_artifact(Path(args.manager_packet_artifact))
    output_path = Path(args.output)

    if args.mode == "live" and not args.allow_live:
        artifact = blocked_live_artifact()
        artifact["failure_family"] = "live_mode_requires_explicit_allow_live"
...
async def _run_live(*, packet_artifact: dict[str, Any]) -> tuple[dict[str, Any], int]:
    adapter = BuilderSpaceAdapter(
        ...
        role_label="websearch_candidate_packet_smoke_manager",
    )
    ...
    for packet_case in packet_artifact.get("cases") or []:
        ...
        try:
            parsed, trace = await adapter.complete_with_trace(
                system_prompt=WEBSEARCH_CANDIDATE_MANAGER_SYSTEM_PROMPT,
...
def _load_manager_packet_artifact(path: Path) -> dict[str, Any]:
    if path.exists():
    ...
    build_manager_packet_smoke(["--output", str(path)])
    return read_json_artifact(path)
