...
DEFAULT_OUTPUT_PATH = ROOT / "artifacts" / "accurate_intake_pl_ce_local_review_evidence_manifest.json"
...
def _missing_payload(group_id: str, path: Path) -> dict[str, Any]:
    return {
        "artifact_type": "missing_pl_ce_local_review_evidence",
        "status": "missing",
        "group_id": group_id,
...
def build_pl_ce_local_review_evidence_manifest(
    *,
    ...
    metadata = {
        ...
        "artifact_type": "accurate_intake_pl_ce_local_review_evidence_manifest",
        "status": status,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Collect local PL+CE review JSON artifacts into decision-pack input."
    )
    parser.add_argument(
        "--artifact",
        action="append",
    ...
    manifest = build_pl_ce_local_review_evidence_manifest(
