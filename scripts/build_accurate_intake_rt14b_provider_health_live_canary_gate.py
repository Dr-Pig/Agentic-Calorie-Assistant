...
DEFAULT_OUTPUT_PATH = ROOT / "artifacts" / "accurate_intake_rt14b_provider_health_live_canary_gate.json"
REQUIRED_STAGE_ID = "provider_health_smoke"
...
def build_rt14b_provider_health_live_canary_gate(
    *,
    ...
    if not stage:
        ...
        if failure_family:
            blockers.append(f"provider_health_failure_family:{failure_family}")
    ...
    return {
        ...
        "artifact_type": "accurate_intake_rt14b_provider_health_live_canary_gate",
        "claim_scope": "manager_runtime_provider_health_live_canary",
        "launch_scope": "current_shell_v1",
        "producer_track": "CurrentShell/ManagerRuntime",
        "target_manager_runtime_gate": "rt14b_provider_health_live_canary",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build the RT14b provider-health live canary gate artifact."
    )
    parser.add_argument(
        "--source-artifact",
        type=Path,
        required=True,
        help="Path to an accurate_intake_mvp_live_diagnostic live provider-health artifact.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_PATH,
        help="Where to write the RT14b gate artifact JSON.",
    )
    args = parser.parse_args(argv)

    source_artifact = json.loads(args.source_artifact.read_text(encoding="utf-8"))
    gate_artifact = build_rt14b_provider_health_live_canary_gate(
