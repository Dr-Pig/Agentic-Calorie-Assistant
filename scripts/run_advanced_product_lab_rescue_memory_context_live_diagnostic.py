...
from app.advanced_shadow_lab.product_lab_rescue_memory_context_artifact import (  # noqa: E402
    blocked_not_invoked_rescue_memory_context_artifact,
)
from app.advanced_shadow_lab.product_lab_rescue_memory_context_live_diagnostic import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run rescue memory context live diagnostic."
    )
        ...
        json.dumps(
            {
                ...
                "live_rescue_memory_context_diagnostic_pass": artifact.get(
                    "live_rescue_memory_context_diagnostic_pass"
                ),
...
def _blocked(
    ...
    artifact = blocked_not_invoked_rescue_memory_context_artifact(
        reason=reason,
        provider_profile_id=str(args.provider_profile_id),
    )
    _attach_preflight(artifact, preflight, args.output)
...
def _live_provider(profile: dict[str, object]) -> Any:
    ...
    return BuilderSpaceAdapter(
        ...
        role_label=str(profile["role_label"]) + "_rescue_memory_context",
    )
