...
from app.advanced_shadow_lab.product_lab_memory_tool_lookup_artifact import (  # noqa: E402
    blocked_not_invoked_memory_tool_lookup_artifact,
)
from app.advanced_shadow_lab.product_lab_memory_tool_lookup_live_diagnostic import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run MemoryRecord-first memory tool lookup live diagnostic."
    )
        ...
        json.dumps(
            {
                ...
                "live_memory_tool_lookup_pass": artifact.get(
                    "live_memory_tool_lookup_pass"
                ),
...
def _blocked(
    ...
    artifact = blocked_not_invoked_memory_tool_lookup_artifact(
        reason=reason,
        provider_profile_id=str(args.provider_profile_id),
    )
    _attach_preflight(artifact, preflight, args.output)
...
def _live_provider(profile: dict[str, object]) -> Any:
    ...
    return BuilderSpaceAdapter(
        ...
        role_label=str(profile["role_label"]) + "_memory_tool_lookup",
    )
