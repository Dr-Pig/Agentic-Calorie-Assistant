...
from app.rescue.application.effective_from_policy import (  # noqa: E402
...
from app.rescue.application.response_presentation_seam import (  # noqa: E402
...
DEFAULT_OUTPUT = ROOT / "artifacts" / "rescue_response_presentation_provider_diagnostic.json"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run rescue response-presentation provider diagnostic.")
    parser.add_argument("--response-presentation-payload", type=Path)
    ...
    payload = (
        read_json_artifact(args.response_presentation_payload)
        if args.response_presentation_payload
    ...
    if args.provider_mode == "live":
        ...
        provider = BuilderSpaceAdapter(
            ...
            role_label="rescue_response_presentation_diagnostic",
        )
        artifact = run_rescue_response_presentation_provider_diagnostic(
            response_presentation_payload=payload,
            provider=provider,
        ...
        artifact = run_rescue_response_presentation_provider_diagnostic(
            response_presentation_payload=payload,
            provider=FakeRescueResponsePresentationProvider(_fixture_candidate()),
            provider_mode="fake",
...
def _fixture_payload() -> dict[str, Any]:
    ...
    shaping_payload = build_rescue_proposal_shaping_payload(
    ...
    shaping_validation = validate_rescue_proposal_shaping_output(
        proposal_shaping_payload=shaping_payload,
        candidate_output=_shaping_candidate(),
    )
    return build_rescue_response_presentation_payload(
        proposal_id="rescue-proposal-fixture",
        proposal_shaping_payload=shaping_payload,
        proposal_shaping_validation=shaping_validation,
        option_generation_result=option_result,
        effective_from_policy=build_rescue_effective_from_policy(
            option_generation_result=option_result,
...
def _option_result() -> dict[str, Any]:
    return {
...
def _shaping_candidate() -> dict[str, Any]:
    return {
...
def _blocked_artifact(
    ...
    return {
        ...
        "status": "blocked",
        "owner": "scripts/run_rescue_response_presentation_live_diagnostic.py",
