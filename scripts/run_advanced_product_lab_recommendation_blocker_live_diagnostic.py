...
from app.advanced_shadow_lab.product_lab_recommendation_blocker_artifact import (  # noqa: E402
    blocked_not_invoked_recommendation_blocker_artifact,
)
from app.advanced_shadow_lab.product_lab_recommendation_blocker_live_diagnostic import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run recommendation blocker live diagnostic."
    )
        ...
        json.dumps(
            {
                ...
                "live_recommendation_blocker_diagnostic_pass": artifact.get(
                    "live_recommendation_blocker_diagnostic_pass"
                ),
...
def _blocked(
    ...
    artifact = blocked_not_invoked_recommendation_blocker_artifact(
        reason=reason,
        provider_profile_id=str(args.provider_profile_id),
    )
    _attach_preflight(artifact, preflight, args.output)
...
def _live_provider(profile: dict[str, object]) -> Any:
    ...
    return BuilderSpaceAdapter(
        ...
        role_label=str(profile["role_label"]) + "_recommendation_blocker",
    )
