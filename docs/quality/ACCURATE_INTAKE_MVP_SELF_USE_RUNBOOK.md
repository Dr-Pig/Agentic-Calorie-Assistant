...
The desktop dogfood entry is available at:

```text
/static/accurate-intake-desktop.html
```

The legacy combined local browser shell is available at:
...
The launcher prints a descriptor with the friendly `/accurate-intake` URL, static `/static/accurate-intake-desktop.html` redirect target, `X-Local-Debug-Token` manual fallback value, persistent SQLite path, and sanitized provider preflight status. Opening the friendly launcher route automatically establishes the loopback-only `local_debug_session` cookie before redirecting to the static desktop entry page; the manual token field remains only as a fallback if the automatic route was not used. The desktop entry page links Chat / Today / Body / Feedback / Review / Data while preserving `user_id` and `local_date`, and it reads `/ping` to show whether Manager / Search / Extract providers are configured without exposing provider topology or secrets. The cookie is server-set, `HttpOnly`, `SameSite=Strict`, host-only, and never placed in the URL or browser storage. This command is a local operator entrypoint only; it does not approve private self-use, product readiness, production DB use, live LLM readiness, or FoodDB expansion.
...
```powershell
python scripts/run_accurate_intake_persistent_desktop_dogfood_baseline.py --db-path workspace_data/local_dogfood/accurate_intake.sqlite3 --local-date YYYY-MM-DD --user-id local-self-use-001 --local-debug-token <token> --feedback-dir workspace_data/local_dogfood_feedback --backup-dir workspace_data/local_dogfood_backups --export-dir workspace_data/local_dogfood_exports --review-queue-artifact-path artifacts/accurate_intake_dogfood_review_queue.json --output artifacts/accurate_intake_persistent_desktop_dogfood_baseline.json
```

This baseline uses the same desktop launcher app path and stable local SQLite DB, then verifies a macro-present approved exact FoodDB packet commit plus ambiguous no-commit across restart. It also checks chat-history trace linkage, Today/debug same-truth, local feedback capture, review-queue ingestion, and export sidecars. Add `--require-browser-execution` when the operator environment has Playwright and the desktop path must be exercised through Chromium navigation instead of route-only `TestClient` calls; that mode also checks the Today macro panel and adjacent-date isolation so a selected next day does not inherit the prior day's consumed kcal or chat history. For disposable PR verification, point the DB and sidecar paths at the same `.pytest_tmp_local` folder so local dogfood feedback is not mixed into test evidence. `--reset-db` is refused for `workspace_data/local_dogfood/accurate_intake.sqlite3` and other real dogfood paths with `backup_required_before_reset`; create a backup first or use a disposable `.pytest_tmp_local` DB. The baseline remains local deterministic dogfood evidence only and must not promote FoodDB truth, canonical eval truth, private self-use approval, product readiness, or live LLM readiness.
