...
Bundle 2 覆蓋旅程 C（珍珠奶茶 logged estimate + follow-up）、D（家常菜 clarify）、E（超標警告）、K（餐點修正）。
...
## Follow-up / Clarify 模式說明

Bundle 2 的 follow-up / clarify cases 分兩種模式。舊 V1 golden wording is compatibility context only; approved B2 product semantics supersede stale draft-first expectations.
...
- `state_delta.canonical_commit` 應為 `true`（缺糖量/杯型的珍珠奶茶已批准為 logged estimate + follow-up）
- turn 1 no longer requires `canonical_commit=false`; follow-up is precision/refinement, not a commit gate

**turn 1 expected_ui_state（logged estimate 狀態）:**
- `today.consumed_kcal > 0`
- `today.active_meal_count >= 1`
- `today.show_macro` 仍受 macro 對齊與 uncertainty 顯示規則約束
...
- 回覆**應**包含更精確的估算值（例如「半糖大杯約 500 kcal，已更新/已記錄」）
- `state_delta.canonical_commit` 應為 `true`
- turn 2 is a refinement/correction of the logged estimate, not the first commit gate
...
| Case | Priority | 通過條件 |
|------|---------|---------|
| C-001 | P0 | Turn 1 logged estimate + 反問糖量/杯型；Turn 2 refinement/correction 並給更精確估算值 |
...
- C-001 Turn 1 的 `state_delta.canonical_commit` 必須是 `true`（logged estimate + follow-up）
- D-001 clarify Turn 1 的 `state_delta.canonical_commit` 必須是 `false`
