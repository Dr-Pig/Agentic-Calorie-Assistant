...
- Default live diagnostic probes use provider request timeout `180000ms`, derived per-case timeout `195000ms`, and provider request retry count `0`.
- The timeout is an observation/failure boundary, not a Current Shell v1 latency target. Do not lower diagnostic timeout values to the product latency SLO; use `latency_slo` to classify slow observations instead. Shorter timeout values are allowed only when explicitly testing user-facing degraded-response deadlines.
...
- `latency_breakdown.by_case_turn_runtime` with each turn split into total, provider invocation, and non-provider runtime latency
- `latency_breakdown.slowest_turn_runtime_segments` before any prompt, provider, timeout, or workflow optimization is attempted
...
```powershell
python scripts/run_accurate_intake_mvp_live_diagnostic.py --stage provider_health_smoke --provider-profile-id builderspace-kimi-k2-5-accurate-intake-mvp-live-diagnostic --provider-timeout-ms 180000 --provider-request-retry-count 0 --output artifacts/accurate_intake_mvp_live_diagnostic_kimi_provider_health.json
python scripts/run_accurate_intake_mvp_live_diagnostic.py --stage schema_contract_probe --provider-profile-id builderspace-kimi-k2-5-accurate-intake-mvp-live-diagnostic --provider-timeout-ms 180000 --provider-request-retry-count 0 --output artifacts/accurate_intake_mvp_live_diagnostic_kimi_schema_probe.json
python scripts/run_accurate_intake_mvp_live_diagnostic.py --stage single_case_live_probe --case-id explicit_item_removal_seeded --provider-profile-id builderspace-kimi-k2-5-accurate-intake-mvp-live-diagnostic --provider-timeout-ms 180000 --provider-request-retry-count 0 --output artifacts/accurate_intake_mvp_live_diagnostic_kimi_seeded_probe.json
```
