# Text Meal DB Wide Research v2.1

## Scope

`v2.1` covers **Base Nutrition shards** only.

It runs after `Source Registry v1` has been reviewed and curated. `v2.1` must only use allowed `base_nutrition` sources from the formal registry:

- `tw-tfda-food-nutrition-composition-dataset`
- `tw-tfda-food-nutrition-database-portal`

`v2.1` does not perform exact-item extraction and does not build meal-pattern records.
...
`v2.1` uses 6 ingredient-family shards:
...
```json
{
  "schema_version": "base-nutrition.v2.1",
  "shard_id": "string",
  "records": [
    {
      "id": "string",
...
- `schema_version` must be `base-nutrition.v2.1`
- no missing outputs
- no empty outputs
- JSON only
...
`v2.1` adds one major rule beyond `v2`: child workers must do **canonical row selection** within TFDA instead of excluding a seed item merely because multiple nearby TFDA rows exist.
