...
> **⚠️ V2 架構注意：** 本文件的 slice 定義（2.1–2.5）是 V1 架構下的工作單元，`required_truth_docs` 中引用了已 archive 的 `L3T_TYPED_RUNTIME_CONTRACT_SPEC.md`。
>
> V2 的 build order 改成 journey bundle（Bundle 0–6），定義在 `APP_V2_TARGET_ARCHITECTURE_SPEC.md` Section 13。
>
> 本文件保留作為 V1 工作歷史記錄，不作為 V2 的 build gate。
