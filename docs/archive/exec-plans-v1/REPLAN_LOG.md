...
## 2026-04-20 — App V2 Cutover Became Eval-Driven

### Trigger

- product direction shifted from pass/workflow-first widening to manager-style App V2 cutover
- `V2_EVAL_BUNDLE_1_CASES.md` was elevated from supporting artifact to the primary Bundle 1 gate
- active execution truth was still pointing at `2.6 calibration`, which no longer matched the intended build sequence

### What The Planning Reality Became

- App V2 must now be driven by journey bundles and executable eval, not by filling out spec detail first
- `Bundle 0` is a truth-cleanup wave only: owner docs, eval gate, execution truth, and architecture vocabulary must align before Bundle 1 runtime begins
- the earlier `2.6 calibration` branch remains valuable prerequisite work, but it is no longer the active architectural critical path

### Assumptions That Expired

- "the next bounded implementation wave should still be calibration + body observation"
- "Bundle 0 can include runtime widening such as `/v2/estimate` while still being called truth cleanup"
- "sidecar / gate vocabulary can stay loosely defined during build start"

### Next-Phase Corrections

- point `CURRENT_EXECUTION_PLAN.md` at `V2 Bundle 0 Truth Cleanup`
- treat `State Resolver` and `Execution Guard` as the only valid replacements for the earlier `Thin State Gate` wording
- treat `Deterministic Sidecar` as a hard architecture rule
- require Bundle 1 runtime work to remain bounded to A/B/J until the eval gate passes
...
## 2026-04-20 — Bundle 0 Truth Cleanup And Bundle 1 Shadow Runtime Landed

### Trigger

- App V2 moved from spec-driven sequencing to eval-driven bundle gating
- the repo needed owner-truth cleanup before any manager-style runtime widening
- after Bundle 0 cleanup, Bundle 1 could be implemented as a shadow runtime on `/v2/estimate` without changing V1 `/estimate` behavior

### What Changed

- Bundle 0 truth cleanup landed:
  - `APP_V2_TARGET_ARCHITECTURE_SPEC.md`
  - `APP_V2_IMPLEMENTATION_PLAN.md`
  - `V2_EVAL_BUNDLE_1_CASES.md`
  - active execution truth now treats Bundle 1 eval cases as the primary gate
- Bundle 1 shadow runtime landed:
  - `app/web/v2_routes.py`
  - `app/application/v2_state_resolver.py`
  - `app/agent/manager.py`
  - `app/application/v2_manager_tools.py`
  - `app/application/v2_execution_guard.py`
  - `app/application/v2_renderer.py`
  - `app/application/v2_sidecar.py`
  - `app/application/v2_bundle1_service.py`
- automated checks now cover Journey A / B / J through:
  - `tests/test_v2_bundle1_service.py`
  - `tests/test_v2_estimate_routes.py`

### Why This Replan Matters

- the repo now has a manager-style shadow trunk that is real enough for founder manual E2E instead of only architecture discussion
- `/v2/estimate` can seed onboarding, log a single meal into canonical state, answer remaining budget from deterministic read surfaces, and fail closed when onboarding is missing
- Bundle 2 intake-depth work can now be gated by direct product feel and chat/UI/state coherence instead of by more speculative spec work
