...
- **`scripts/check_executable_action_pack_contract.py`**: CI/pre-commit hard gate for derived executable action packs; enforces that executable runner-input artifacts stay subordinate to official utterance packs and inherit the approved primary outcome fields without inventing new truth.
