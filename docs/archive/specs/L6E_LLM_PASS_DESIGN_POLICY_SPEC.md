...
> **⚠️ V2 架構注意：** 本文件的 pass-centered 設計原則（graph-first、pass count governance）在 V2 架構下已被 `APP_V2_TARGET_ARCHITECTURE_SPEC.md` 取代。V2 採用 manager + tools 架構，不再以 pass graph 為核心設計單位。
>
> 本文件仍保留作為參考，其中 **Canonical Matrix（Section 3）** 和 **Decision Mode Rule（Section 2.2A）** 的 deterministic-first 原則仍有效，但 pass count 和 graph 設計已不適用。
>
