...
**Observed**: Agent uses `dict[str, Any]` or untyped returns on pass outputs, commit candidates, or proposal options that have a defined contract.

**Why it's slop**: Typed contracts exist for tool I/O, manager decision envelope, and renderer output. Using `Any` defeats schema validation and makes deterministic gate checks impossible.

**Correct pattern**: Check `APP_V2_TARGET_ARCHITECTURE_SPEC.md` Section 12 for the schema reduction policy. V2 schema only covers: canonical state, tool I/O, manager decision envelope, renderer output envelope, eval fixtures.
...
**Why it's slop**: The deterministic layer may validate, reject, or request one bounded repair round. It may not silently substitute a new semantic judgment. In V2, the Execution Guard is the only deterministic layer after the manager, and it only does legality/safety checks.
...
**Why it's slop**: These distinctions belong in response realization (Renderer), not routing taxonomy. In V2, the Primary Manager is the only semantic control surface — it should not be burdened with response-style distinctions.
