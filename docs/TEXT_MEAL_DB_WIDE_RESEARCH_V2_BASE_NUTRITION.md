# Text Meal DB Wide Research v2

## Scope

`v2` covers **Base Nutrition shards** only.

It runs after `Source Registry v1` has been reviewed and curated. `v2` must only use allowed `base_nutrition` sources from the formal registry:

- `tw-tfda-food-nutrition-composition-dataset`
- `tw-tfda-food-nutrition-database-portal`

`v2` does not perform exact-item extraction and does not build meal-pattern records.
...
`v2` uses 6 ingredient-family shards:
...
```json
{
  "schema_version": "base-nutrition.v2",
  "shard_id": "string",
  "records": [
    {
      "id": "string",
...
- `schema_version` must be `base-nutrition.v2`
- no missing outputs
- no empty outputs
- JSON only
