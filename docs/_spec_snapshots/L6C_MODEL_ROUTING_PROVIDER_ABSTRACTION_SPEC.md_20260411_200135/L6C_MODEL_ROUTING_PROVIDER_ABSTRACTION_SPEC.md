...
若本節與 [`L6D LLM Pass Design Policy Spec`](/C:/Users/User/Documents/Playground/line-liff-calorie-helper-text-meal-canary-main/docs/specs/L6D_LLM_PASS_DESIGN_POLICY_SPEC.md) 或 domain-specific `L3.x` spec 的 canonical graph 發生衝突，應以後者為準。
...
canonical default 依 `L6D` 採 3-node graph；以下是 expanded mode 下各 named pass 的 role mapping。

- `recommendation_context_pass` -> `fast_router_model`
- `candidate_generation_pass` -> `fast_router_model`
...
canonical default 依 `L6D` 採 2-node graph；以下是 expanded mode 下各 named pass 的 role mapping。

- `proposal_gate_pass` -> `strict_reasoner_model`
...
canonical default 依 `L6D` 採 2-3 node graph；以下是 expanded mode 下各 named pass 的 role mapping。

- `rescue_trigger_pass` -> `fast_router_model`
- `rescue_assessment_pass` -> `strict_reasoner_model`
