...
### 與 Manager-style diagnostic packs 的關係

- Manager-style diagnostic packs 應直接引用本 rubric 的 hard-fail / scoring rules
- diagnostic packs 只需要補該 capability gate 的 case-level oracle 與 expected behavior
...
以下是 family-level 重點。Manager-style diagnostics 可只引用當前 family 相關段落。
...
- Manager-style diagnostic packs 可直接引用本 rubric 建立 case-level verdict format
...
2. 為 Manager-style diagnostic packs 加入：
   - hard gate coverage
...
- 2026-04-24: v1 初始版本，建立 whole-product grading rubric，供 diagnostic gates、founder review、baseline comparison、regression triage 使用
