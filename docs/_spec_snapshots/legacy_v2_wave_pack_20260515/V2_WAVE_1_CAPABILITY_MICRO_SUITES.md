...
- 讓 implementation 不只依賴 package-level end-to-end 驗收
- 幫助 coding agent 以 divide-and-conquer 的方式建置，而不是為了讓 historical acceptance packages 變綠做局部 patch
...
- 這些 micro-suites 與 Manager-style diagnostics / benchmark v1/v2 / turn2 replay 的關係是什麼
...
- Manager-style diagnostic readiness checks
...
3. **先過 micro-suites，再跑 Manager-style diagnostics**
   - package-level diagnostics 不是 build order；micro-suites 才是 divide-and-conquer 施工順序
...
### Layer B — Manager-Style Diagnostic Acceptance
...
## 與 Manager-Style Diagnostics / Benchmark / Replay 的關係

## Intake-entry diagnostic 主要依賴
- MS1
- MS7
- MS9
- MS11
...
### 規則 1
單一 micro-suite 不穩時，不應直接一直 patch package-level diagnostic。

### 規則 2
若 intake-depth diagnostic fail，應先回映到哪個 micro-suite contract 壞掉。

### 規則 3
若某個 fix 只能讓 diagnostic case 綠，但對應 micro-suite 沒改善，應視為可疑 patch。
...
- intake-entry diagnostic / intake-depth diagnostic / benchmark / replay 已有對位方向
...
2. 將 current Manager-style diagnostic cases 逐項 map 回對應 micro-suites
