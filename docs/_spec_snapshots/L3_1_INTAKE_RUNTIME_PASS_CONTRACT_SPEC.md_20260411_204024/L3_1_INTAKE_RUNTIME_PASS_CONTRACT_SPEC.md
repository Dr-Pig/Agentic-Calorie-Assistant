...
- `decision_mode: llm`
- `decision_reason: 輸入以自然語言意圖、meal boundary、same-meal vs new-meal 判斷為主，屬模糊語義解讀`
...
- `decision_reason: clarify / tool lookup / proceed 需要語義判斷，但 tool legality 與 blocking semantics 受 deterministic contract 約束`
...
- 判斷 `commit_readiness`
- `decision_mode: llm`
- `decision_reason: 在不完整 evidence 下做 portion 與 nutrition synthesis，屬估算型判斷；commit legality 仍需受 deterministic invariant 約束`
...
- `decision_mode: llm`
- `decision_reason: 此步驟主要負責 user-facing wording、follow-up phrasing、與 correction acknowledgement`
