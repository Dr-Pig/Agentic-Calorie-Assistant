...
- [docs/index.md](/C:/Users/User/Documents/Playground/line-liff-calorie-helper-text-meal-canary-main/docs/index.md) identifies where active execution artifacts live
- [docs/AGENT_LOADING_PATH.md](/C:/Users/User/Documents/Playground/line-liff-calorie-helper-text-meal-canary-main/docs/AGENT_LOADING_PATH.md) points new agents to the correct task and handoff path
...
`tests_to_run[]` entries must be executable commands only. Do not append natural-language qualifiers such as `only if needed`, `for reference`, or `when applicable` on the same line. Conditional guidance belongs in prose outside the executable list.
...
When a task is marked `COMPLETED`, the normal target state is:

1. move the task artifact from `docs/exec-plans/active/tasks/` to `docs/exec-plans/completed/tasks/`
2. move any matching handoff from `docs/handoff/active/` to `docs/handoff/completed/`
3. update any active-plan links that still point at the old active-task location

Short transition windows are acceptable during the same working pass, but completed artifacts should not accumulate indefinitely under `active/`.

The repository gate should treat lingering completed task artifacts under `active/tasks/` or completed-task handoffs under `handoff/active/` as a validation failure, not just a cleanup suggestion.
