...
- [docs/AGENT_LOADING_PATH.md](/C:/Users/User/Documents/Playground/line-liff-calorie-helper-text-meal-canary-main/docs/AGENT_LOADING_PATH.md) defines what to read before entering execution work
...
- Do not treat a near-total staged rewrite of protected entrypoint or governance docs as a normal edit; if the rewrite-churn guard trips, stop and either reduce the change to section-level patches or record an approved rewrite path.
...
Some files are beyond normal design targets but are not yet being split in the current slice.

These files should enter freeze-growth mode instead of continuing silent expansion.

Current freeze-growth files:

- `app/application/evidence_assembly.py`
- `app/application/context_assembly.py`
- `app/agent/knowledge_packets.py`

Current watchlist file:

- `app/agent/nutrition_engine.py`

Rules:

- freeze-growth files must not grow during ordinary feature work
- allowed changes are limited to shrink-only extraction, contained bug fixes, or boundary-safe wiring
- if a freeze-growth file is touched, the task artifact or re-plan note must name the file and classify the change as shrink-only extraction, contained bug fix, or boundary-safe wiring
- watchlist files should appear in workspace audit output even when they are not yet block-gated

The staged fat-file gate treats missing freeze-growth justification as a violation, not a reminder.
...
When a task is marked `COMPLETED`, its completion fields should be written as structured lists/keys instead of free-form prose wherever the protocol defines list-shaped fields.

Repository task governance is CI-enforced in audit mode. `COMPLETED` task artifacts must carry non-empty `actual_touch_files[]` and `tests_run[]` lists.

## Migration Governance Rule

Alembic is the canonical schema-change path.

Current hard rule:

- if `app/models.py` changes, the same change set must include at least one migration file under `alembic/versions/`

The CI harness enforces this through `scripts/check_migration_discipline.py`.
...
- `AGENTS.md` stays short and points into durable docs
