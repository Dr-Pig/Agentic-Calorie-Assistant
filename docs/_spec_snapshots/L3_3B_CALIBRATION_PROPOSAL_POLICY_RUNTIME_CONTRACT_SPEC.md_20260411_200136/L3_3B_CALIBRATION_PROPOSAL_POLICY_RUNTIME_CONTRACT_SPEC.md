...
`L3.3B` 的 canonical default 應採 2-node graph：

1. `proposal_policy_shaping`
2. `proposal_response`

補充規則：

- `proposal eligibility`、`blocked option families`、`budget delta math`、`effect payload legality` 應優先 deterministic-first
- 當 option family 很少且 effect payload 已模板化時，可直接 collapse 成 1 個 LLM-backed proposal shaping node 加 1 個 response node
- cross-domain 原則見 [`L6D LLM Pass Design Policy Spec`](/C:/Users/User/Documents/Playground/line-liff-calorie-helper-text-meal-canary-main/docs/specs/L6D_LLM_PASS_DESIGN_POLICY_SPEC.md)
...
expanded mode 下建議的 logical model role 對應：
