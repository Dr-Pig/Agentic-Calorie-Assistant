...
- `/calibration/proposal/stored-action` response `state_delta` owns stored-action mutation reporting. PL/CE may render `state_delta.plan_mutated` and `state_delta.ledger_mutated`, but must not infer mutation state from action labels, proposal status, button text, route position, or card ordering.
