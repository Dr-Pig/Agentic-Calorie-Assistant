...
| `body_budget_effective_budget_view` | `/today/effective-budget`; `app.composition.body_budget_effective_budget.build_body_budget_effective_budget_view` | `budget` owns ledger math; composition exposes adjustment-layer read model and sign-policy trace | Render backend-supplied effective budget posture, adjustment layer totals, current runtime sign policy, and canonical formula migration status. | `source_kind`, `read_only`, `truth_owner`, `ledger_present`, `base_budget_kcal`, `consumed_kcal`, `runtime_adjustment_total_kcal`, `runtime_effective_budget_kcal`, `remaining_kcal`, `remaining_formula`, `adjustment_layers`, `entry_breakdown`, `sign_policy`, `calibration_adjustment_ledger_entry_enabled` | Do not calculate effective budget, adjustment layer totals, or sign policy in PL/CE; do not infer calibration adjustment ledger-entry readiness from this read model; do not switch to canonical L3M plus-signed formula until a separate mutation/math migration slice lands. | `tests/test_body_budget_effective_budget_read_model.py`; `/today/effective-budget` smoke |
