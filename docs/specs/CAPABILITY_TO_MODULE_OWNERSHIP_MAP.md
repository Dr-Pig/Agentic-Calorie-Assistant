...
- `tests/test_architecture_dependency_debt_audit.py`
...
|---|---|---|---|---|---|
| Intake manager loop, meal draft, commit boundary | `app/intake` with `app/runtime` contracts | CurrentShell active runtime | `app/routes.py`, root `app/schemas.py`, provider adapters | Active CurrentShell only | `intake_turn_orchestrator`, `intake_execution_orchestrator` |
| Nutrition evidence, synthesis, final mapping | `app/nutrition` | CurrentShell ManagerRuntime mainline | `app/runtime` semantic ownership, provider adapters | Active ManagerRuntime mainline | local synthesis and evidence path work |
| Budget ledger and today sync | `app/budget` | CurrentShell active runtime | UI renderer, root facades, unrelated domains | Active CurrentShell only | today budget read path |
| Body plan, observation, calibration baseline | `app/body` | CurrentShell active runtime plus calibration proposal gates | ledger shortcuts, recommendation, rescue | Active baseline plus calibration proposal seam only | body plan routes and onboarding service |
| Retrieval and food knowledge metadata | `app/knowledge` or `app/nutrition` by source role | Spec or CurrentShell/FoodDB mainline only | provider transport semantics, eval fixture shape | Contract-backed CurrentShell/FoodDB work only | knowledge skeleton, metadata specs |
...
## CurrentShell-Safe Parallel Readiness

This table records the current activation cap for parallel capability work while CurrentShell self-use MVP local desktop dogfood is the mainline. It is an implementation sequencing guard, not permission to activate product behavior.
...
| S7 | Recommendation quality gate | `offline_only` | `app/recommendation` | prepared-candidate quality gate | live search, Google Places, ranking LLM, evidence object dependency |
