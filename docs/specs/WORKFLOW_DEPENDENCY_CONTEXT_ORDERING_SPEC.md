...
*   **邊界補充**：`2.2` 的 correction 僅負責 active meal continuation、明確目標的 correction、以及 local-date attribution。模糊的跨日或跨週歷史召回，不屬於這一層，應留給後續的 memory / retrieval deepening。
...
*   **依賴補充**：`2.5` 依賴的是 canonical ledger truth 與穩定的 read-side surfaces，不依賴 Today UI 的 route 或 presentation behavior。Rescue 仍是 write-side workflow，只能讀取穩定狀態，不能反向依賴 UI 形狀。
