...
### 9.2A v1 `recommended_target_kcal` deterministic baseline

在 v1 尚未完成 calibration 收斂前，系統可先用 deterministic baseline 產生 `recommended_target_kcal`。

目前正式採用：

- `Mifflin-St Jeor` 作為 baseline BMR formula
- coarse activity multiplier 作為活動量近似
- `7700 kcal / kg` 將每週減重目標換算成每日 deficit

計算後規則：

- `recommended_target_kcal = max(BodyPlan.safety_floor_kcal, raw_target_kcal)`
- `recommended_target_kcal` 是 operating baseline，不是醫療定律
- `BodyPlan.safety_floor_kcal` 仍是 hard lower bound，不可被這個公式覆蓋

說明：

- 這是 v1 deterministic baseline
- 後續 calibration 可根據 intake history、body-weight trend、logging quality 改寫 operating target
- 但 calibration 不應把 hard floor 與 personalized target 混成同一個概念
