...
- `deterministic_proposal_gate`
- `proposal_policy_shaping`
- optional `proposal_response`

以下只映射真正的 LLM-backed nodes。
...
- `proposal_gate` / `proposal_gate_pass` 若存在，保持 deterministic，不映射 LLM role
