...
def structured_correction_operation(payload: dict[str, Any] | None) -> str:
    ...
    if isinstance(top_target, dict):
        candidates.extend([top_target.get("correction_operation"), top_target.get("operation")])
    answer_contract = payload.get("answer_contract")
    if isinstance(answer_contract, dict):
        candidates.extend([answer_contract.get("correction_operation"), answer_contract.get("operation")])
    semantic_decision = payload.get("semantic_decision")
    if isinstance(semantic_decision, dict):
        candidates.extend([semantic_decision.get("correction_operation"), semantic_decision.get("operation")])
        semantic_target = semantic_decision.get("target_attachment")
        if isinstance(semantic_target, dict):
            candidates.extend([semantic_target.get("correction_operation"), semantic_target.get("operation")])
