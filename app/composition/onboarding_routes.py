...
async def onboarding_bootstrap(req: OnboardingRequest, db: Any = Depends(get_db)) -> dict:
    ...
    inputs = OnboardingBootstrapInput(
        ...
        activity_level=(req.activity_level or "sedentary"), # type: ignore
        daily_lifestyle=req.daily_lifestyle, # type: ignore
        weekly_exercise_days_band=req.weekly_exercise_days_band, # type: ignore
