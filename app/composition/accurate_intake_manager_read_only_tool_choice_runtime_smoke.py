...
def _validate(cases: list[dict[str, Any]]) -> list[str]:
    ...
    for case in cases:
        ...
        if state_mutation.get("canonical_commit") is not False:
            blockers.append(f"{case_id}.canonical_commit_changed")
        if state_mutation.get("ledger_updated") is not False:
            blockers.append(f"{case_id}.ledger_updated")
        if state_mutation.get("meal_logged") is not False:
            blockers.append(f"{case_id}.meal_logged")
...
async def build_manager_read_only_tool_choice_runtime_smoke_artifact() -> dict[str, Any]:
    ...
    artifact = {
        ...
        "artifact_type": "accurate_intake_manager_read_only_tool_choice_runtime_smoke",
        "status": "manager_read_only_tool_choice_runtime_smoke_pass",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "claim_scope": "current_shell_non_fooddb_read_only_runtime_tool_choice_smoke",
