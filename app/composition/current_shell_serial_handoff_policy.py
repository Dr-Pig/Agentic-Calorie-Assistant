...
def _queue_blockers(queue_metadata: dict[str, Any]) -> list[str]:
    ...
    if not matches_alias(
        ...
        *LEGACY_MERGE_QUEUE_METADATA_ARTIFACT_TYPES,
        ...
        blockers.append(f"queue_metadata.unexpected_artifact_type:{queue_metadata.get('artifact_type')}")
    if queue_metadata.get("metadata_source") not in ALLOWED_QUEUE_METADATA_SOURCES:
        blockers.append(f"queue_metadata.untrusted_metadata_source:{queue_metadata.get('metadata_source')}")
