...
def active_meal_context_allowed(planner_result: TurnIntentResult) -> bool:
    return planner_result.meal_boundary == "continue_active_meal"


def build_canonical_meal_state(
    *,
    meal_id: int | None,
    meal_title: str,
    status: MealStatus,
...
def canonical_meal_state_from_runtime(
    *,
    ...
    return build_canonical_meal_state(
        meal_id=meal_id,
        meal_title=meal_title,
...
def build_boundary_trace(
    *,
    ...
    resolution_state = (
        "resolved"
        if state.boundary_clarification_open and planner_result.meal_boundary != "boundary_clarification"
        else "open"
        if planner_result.meal_boundary == "boundary_clarification"
        else "not_applicable"
    )
    return {
        "meal_boundary": planner_result.meal_boundary,
        "planner_self_reported_confidence": (confidence_signals or {}).get(
            "planner_self_reported_confidence", planner_result.boundary_confidence
        ),
        "calibrated_confidence": planner_result.boundary_confidence,
        ...
        "boundary_followup_triggered": planner_result.meal_boundary == "boundary_clarification",
