...
def _build_span_timeline(
    *,
    ...
    for idx, trace in enumerate(llm_traces):
        ...
        spans.append(
            TraceSpan(
                step_name=str(trace.get("stage") or "unknown"),
                span_id=span_id,
    ...
    for index, attempt in enumerate(grounding_attempts, start=1):
        spans.append(
            TraceSpan(
                step_name=f"grounding_{attempt.get('kind', 'unknown')}",
    ...
    for index, step in enumerate(enrichment_steps, start=1):
        spans.append(
            TraceSpan(
                step_name="deterministic_enrichment",
    ...
    if persistence:
        spans.append(
            TraceSpan(
                step_name="persistence_decision",
