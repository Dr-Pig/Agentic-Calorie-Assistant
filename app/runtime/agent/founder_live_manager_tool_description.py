...
def founder_live_manager_tool_description() -> str:
    return (
        ...
        "When answering a target clarification, a meal slot such as 早餐, 午餐, 中餐, 晚餐, or 宵夜 uniquely selects a meal_thread candidate if exactly one candidate target_display_name/display_name/meal_title contains that slot; call resolve_correction_target with that meal_thread_id and target_display_name instead of repeating the clarification. "
        ...
        "If manager_contract_evidence_state.target_validation_failure_family is manager_thread_target_proposal_ambiguous, retry resolve_correction_target only when the current turn uniquely identifies one context candidate by meal title/slot/time/item; a clarification answer with one slot-matched meal_thread candidate is unique and should retry with that candidate id/name. Otherwise ask a target clarification with no mutation and do not retry the same rejected target. "
