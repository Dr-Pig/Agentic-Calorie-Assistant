...
def _exact_variant_type(candidate: dict[str, Any]) -> str:
    ...
    if any(token in combined for token in ("ml", "nutrition label", "foodtracer", "瓶", "罐", "pack", "packaged", "bottle", "can")):
        return "packaged_retail"
    if any(token in combined for token in ("焦糖", "草莓", "抹茶", "榛果", "太妃", "香草", "摩卡", "flavor", "flavored", "seasonal")):
...
def _exact_identity_gate(candidate: dict[str, Any], *, query: str) -> bool:
    ...
    if variant_type == "packaged_retail" and query_alignment == "weak":
        return False
    if identity_confidence == "low" and query_alignment == "weak":
        return False
    ...
    if variant_type == "packaged_retail" and not any(token in query_text for token in ("ml", "瓶", "罐", "包", "pack", "bottle", "can")):
        return query_alignment in {"exact_title", "exact_alias"} and any(token in query_text for token in ("7-11", "全家", "familymart", "超商", "starbucks", "mcdonald", "mos"))
    return True
    ...
    *,
    ...
    for candidate in raw_candidates:
        augmented = _augment_exact_candidate(
            candidate | {"aliases": [str(candidate.get("title") or "")]},
            query=search_query,
